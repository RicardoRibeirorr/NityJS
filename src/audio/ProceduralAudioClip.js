...
export class ProceduralAudioClip {
     ...
     * @param {string} filename - Optional filename (defaults to clip name)
     */
    downloadAsWAV(filename) {
        if (!this.isLoaded()) {
            console.error('Audio must be loaded before downloading');
            return;
        }
        ...
        console.log(`Downloaded audio as: ${actualFilename}`);
    }
    ...
    audioBufferToWAV(buffer) {
        ...
        // RIFF header
        writeString(0, 'RIFF');
        writeUint32(4, 36 + length * channels * 2);
        writeString(8, 'WAVE');
        ...
        writeUint16(22, channels);              // number of channels
        writeUint32(24, sampleRate);            // sample rate
        writeUint32(28, sampleRate * channels * 2); // byte rate
        writeUint16(32, channels * 2);          // block align
        writeUint16(34, 16);                    // bits per sample
        
        // data chunk
        writeString(36, 'data');
        writeUint32(40, length * channels * 2);
